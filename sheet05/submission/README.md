...
specified set of particles.
    * So in the end the simulation [`membrane_cloth.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#membrane_cloth.mp4)
...
because we figured that an outflow boundary just does not make a lot of sense in the same setting as membranes
   * We did not have these problems with reflective or even periodic boundaries. An example of a simulation done with periodic boundaries is the video called [`membrane_periodic_cloth.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#membrane_periodic_cloth.mp4).
...
end up differently every time, due to floating point rounding errors. You can observe it in the [`parallelization_chaotic_system_particle_locking.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#parallelization_chaotic_system_particle_locking.mp4) video, where we ran the same simulation a bunch of times with different thread counts and overlayed the different runs.
...
3. **Performance-Tests**
   ...
   * If you now compare the graphs in [`Rayleigh_Taylor_3D_Speedup_Comparison.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#Rayleigh_Taylor_3D_Speedup_Comparison.png), the speedup is almost identical for both methods, which is misleading when analysing the performance. Despite the rather similar speedup, the graphs in [`Rayleigh_Taylor_3D_Time_Comparison.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#Rayleigh_Taylor_3D_Time_Comparison.png) show, that the runtime for the particle locking method is actually around twice as high as for the domain partitioning.
   ...
   * To get an in depth profile of the program, we ran vtune and seem to have optimised the parallelization quite well, as seen in [`VTuneHotspots_Parallelization_V1.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#VTuneHotspots_Parallelization_V1.png) and the `/../data/vtune*/` folder.
   * On the CoolMUC the MUP/s plummet for more than 42 threads for both versions as seen in [`Rayleigh_Taylor_3D_MUPs_Comparison.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#Rayleigh_Taylor_3D_MUPs_Comparison.png) . This
...
2. **Simulations**
   * The first simulation we ran was without any obstacle accelerated by gravity and named [`nano_scale_flow_short.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#nano_scale_flow_short.mp4).
Its velocity profile, [`nano_Velocity_Profile_No_Obstacle.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#nano_Velocity_Profile_No_Obstacle.png), looks smooth as expected, and shows lower velocities at the walls of the tube (caused by some kind of friction).
The curves, representing the different timeframes, seem to show this fact more clearly over time.
   * The second simulation was with a fixed cuboid obstacle on the side and named [`nano_scale_flow_short_obstacle_cuboid.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#nano_scale_flow_short_obstacle_cuboid.mp4).
Its velocity profile, [`nano_scale_flow_short_obstacle_cuboid.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#nano_scale_flow_short_obstacle_cuboid.mp4), seems to be held up starting approximately at iteration (time) 12000.
So a lot of particles are held up by a cuboid, so we wanted to try to see the effects of another shape.
   * The third simulation was with a fixed sphere obstacle on the side and named [`nano_scale_flow_short_obstacle.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#nano_scale_flow_short_obstacle.mp4).
In its velocity profile, [`nano_Velocity_Profile_Sphere_Obstacle.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#nano_Velocity_Profile_Sphere_Obstacle.png), we can see how the flow is not as abruptly held up as by the cuboid.
...
So we ran a simulation [`reynolds.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#reynolds.mp4) with a higher initial velocity and a cuboid object. That simulation can be fond at `input/nano_scale_flow/nano_scale_flow_higher_reynolds_number.xml`. We wanted to see how the initial velocity affects the flow and the velocity profile in the end.
...
2. **Simulations:**
   ...
   * We can observe how the supercooling leads to more but smaller holes in the structure and you can also observe this in the quantitative analysis. In [`ArgonRDF_SuperCooling.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#ArgonRDF_SuperCooling.png) the spikes in local density are not as regular as the ones in the slow cooling plot. This is an indicator for a more unsorted structure emerging when super-cooling. Also in the diffusion plot [`ArgonDiffusion.png`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#ArgonDiffusion.png) you can clearly see how strongly diffusion and temperature are correlated, when you compare this plot to the temperature history.
...
* The 3D Rayleigh-Taylor Instability simulation from Contest 2 can be found in [`rayleigh_taylor_instability_3d.mp4`](https://manuellerchner.github.io/MolSim-WS23-24/submissions/#sheet05#rayleigh_taylor_instability_3d.mp4)
